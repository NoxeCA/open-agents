...
export const bomItemSchema = z.object({
  ...
  extendedDescription: z.string().transform(val => val || undefined).optional(),
...
export const serviceSectionSchema = z.object({
  ...
  layout: z.enum(['zero-ventilation', 'itemized-without-price', 'itemized-with-price']),
});

export const serviceSectionPageSchema = z.object({
  projectTitle: z.string().min(1, 'Project title is required'),
  projectIntro: z.string().min(1, 'Project intro is required'),
  services: z.array(serviceSectionSchema).min(1, 'At least one service section is required'),
});
