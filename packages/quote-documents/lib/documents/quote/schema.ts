import { z } from "zod";
import {
  buildQuoteDocumentOptionalPageRegionTarget,
...
const quoteDataSchemaBase = metaSchema
...
export const quoteDataSchema = quoteDataSchemaBase.superRefine((quote, ctx) => {
  ...
  for (const [index, page] of (quote.optionalPages ?? []).entries()) {
    const regionId = buildQuoteDocumentRegionId(
      buildQuoteDocumentOptionalPageRegionTarget(page, index, "body"),
    );
